<template>
  <DashboardLayout>
    <div class="mb-6 flex justify-between items-center">
      <div>
        <h1 class="text-3xl font-extrabold text-gray-900">Machine Diagnostics (V1.0)</h1>
        <p class="text-base text-gray-600 mt-1">Monitor startup sensors and perform manual hardware tests for 1.0 Hardware.</p>
      </div>
...
import { useUserStore } from '@/stores/userStore'; // 👈 Added user store import

// Initialize user store and role to match MachineManagementView
...
// Fetch operational machines list matching the concept from MachineManagementView
const fetchMachines = async () => {
  try {
    // Uses operational endpoints instead of manufacturer warehouse endpoints
    const endpoint = userRole === 'Owner' ? '/api/machine/owner' : '/api/machine/all';
    const response = await api.get(endpoint); 
    
    // Map the properties safely to fit the template select fields
    machines.value = response.data.map(m => ({
      id: m.machineId || m.id, 
      label: m.location?.name ? `${m.machineId} - ${m.location.name}` : m.machineId || m.id
    }));
    ...
    console.error("Failed to fetch operational machines list:", error);
  }
};
